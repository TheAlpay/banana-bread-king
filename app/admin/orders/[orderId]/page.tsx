...
export default function AdminOrderDetailPage() {
  ...
  if (loading) return <div className="py-16 text-center text-[#B89878]">Loading...</div>
  if (!order) return <div className="py-16 text-center text-[#B89878]">Order not found.</div>

  return (
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-playfair text-3xl font-bold text-[#5C2B0F]">Order {order.invoiceNumber}</h1>
          <p className="text-[#B89878] text-sm mt-1">{new Date(order.createdAt).toLocaleString('en-AU')}</p>
        </div>
        <Link href="/admin/orders" className="text-sm text-[#5C2B0F] hover:underline">← Orders</Link>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        {/* Customer info */}
        <div className="bg-white rounded-2xl shadow-sm p-5">
          <h2 className="font-semibold text-[#1C0A00] mb-3 text-sm">Customer</h2>
          <div className="text-sm space-y-1 text-[#7A5A42]">
            <p className="font-medium text-[#1C0A00]">{order.userName}</p>
            {order.userCompany && <p>{order.userCompany}</p>}
            <p>{order.userEmail}</p>
            <Link href={`/admin/users/${order.userId}`} className="text-[#5C2B0F] hover:underline text-xs">View profile →</Link>
          </div>
        </div>

        {/* Status */}
        <div className="bg-white rounded-2xl shadow-sm p-5">
          <h2 className="font-semibold text-[#1C0A00] mb-3 text-sm">Status</h2>
          <select
            value={order.status}
            onChange={(e) => updateStatus(e.target.value as OrderStatus)}
            disabled={statusUpdating}
            className="w-full border border-[#E8D5B8] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#5C2B0F]/30 capitalize"
          >
        ...
        {/* Shipping */}
        <div className="bg-white rounded-2xl shadow-sm p-5">
          <h2 className="font-semibold text-[#1C0A00] mb-3 text-sm">Shipping</h2>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
        <h2 className="font-semibold text-[#1C0A00] mb-4">Items</h2>
        <table className="w-full text-sm">
          <thead><tr className="text-left text-[#A08060] border-b border-[#F2E4CE]">
        ...
        <div className="border-t border-[#F2E4CE] mt-4 pt-4 text-sm space-y-1 text-right">
          <div className="flex justify-end gap-8 text-[#A08060]"><span>Subtotal</span><span>${(order.subtotal / 100).toFixed(2)}</span></div>
          ...
          <div className="flex justify-end gap-8 text-[#A08060]"><span>GST</span><span>${(order.gst / 100).toFixed(2)}</span></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-sm p-5">
          <h2 className="font-semibold text-[#1C0A00] mb-4 text-sm">Invoice</h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5">
          <h2 className="font-semibold text-[#1C0A00] mb-3 text-sm">Internal Notes</h2>
          <textarea
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            rows={3}
            placeholder="Admin notes (not visible to customer)..."
            className="w-full border border-[#E8D5B8] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#5C2B0F]/30 resize-none mb-2"
          />

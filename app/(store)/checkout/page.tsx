...
export default function CheckoutPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      <h1 className="font-playfair text-3xl font-bold text-[#8B4513] mb-8">Checkout</h1>
      ...
      <form onSubmit={handleCheckout} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {isWholesale && (
            <label className="flex items-center gap-3 cursor-pointer bg-[#fdf8f0] rounded-xl p-4">
              <input
                type="checkbox"
              ...
              />
              <div>
                <p className="text-sm font-medium text-gray-700">Request invoice (Net 30)</p>
                <p className="text-xs text-gray-400">Our team will contact you for payment arrangements.</p>
              </div>
            </label>
          )}
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5 space-y-3">
            {items.map((item) => (
              <div key={`${item.productId}-${item.variety}`} className="flex justify-between text-sm">
                <span className="text-gray-600">{item.name} ({item.variety}) × {item.quantity}</span>
